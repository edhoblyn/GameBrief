module EventImporters
  class Battlefield6EventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["battlefield-6"].freeze
    GAME_NAMES = ["Battlefield 6"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Battlefield 6 game not found" if game.nil?

      all_results = EventScrapers::Battlefield6EventScraper.new.call
