module EventImporters
  class CallOfDutyWarzoneEventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["call-of-duty-warzone", "warzone"].freeze
    GAME_NAMES = ["Call of Duty: Warzone", "Warzone"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Call of Duty: Warzone game not found" if game.nil?

      all_results = EventScrapers::CallOfDutyWarzoneEventScraper.new.call
