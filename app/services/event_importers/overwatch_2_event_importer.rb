module EventImporters
  class Overwatch2EventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["overwatch-2", "overwatch"].freeze
    GAME_NAMES = ["Overwatch 2", "Overwatch"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Overwatch 2 game not found" if game.nil?

      all_results = EventScrapers::Overwatch2EventScraper.new.call
