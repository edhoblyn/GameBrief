module EventImporters
  class Destiny2EventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["destiny-2"].freeze
    GAME_NAMES = ["Destiny 2"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Destiny 2 game not found" if game.nil?

      all_results = EventScrapers::Destiny2EventScraper.new.call
