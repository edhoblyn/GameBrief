module EventImporters
  class PubgEventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["pubg-battlegrounds", "pubg"].freeze
    GAME_NAMES = ["PUBG: Battlegrounds", "PUBG", "PlayerUnknown's Battlegrounds"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "PUBG: Battlegrounds game not found" if game.nil?

      all_results = EventScrapers::PubgEventScraper.new.call
