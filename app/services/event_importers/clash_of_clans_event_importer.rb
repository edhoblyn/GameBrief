module EventImporters
  class ClashOfClansEventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["clash-of-clans"].freeze
    GAME_NAMES = ["Clash of Clans"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Clash of Clans game not found" if game.nil?

      all_results = EventScrapers::ClashOfClansEventScraper.new.call
