module EventImporters
  class Dota2EventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["dota-2"].freeze
    GAME_NAMES = ["Dota 2"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Dota 2 game not found" if game.nil?

      all_results = EventScrapers::Dota2EventScraper.new.call
