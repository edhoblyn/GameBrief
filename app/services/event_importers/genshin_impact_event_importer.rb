module EventImporters
  class GenshinImpactEventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["genshin-impact"].freeze
    GAME_NAMES = ["Genshin Impact"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Genshin Impact game not found" if game.nil?

      all_results = EventScrapers::GenshinImpactEventScraper.new.call
