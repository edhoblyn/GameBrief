module EventImporters
  class CounterStrike2EventImporter
    Result = Struct.new(:imported, :skipped, keyword_init: true)

    GAME_SLUGS = ["counter-strike-2", "cs2"].freeze
    GAME_NAMES = ["Counter-Strike 2", "CS2"].freeze

    def call(replace: false)
      game = find_game
      raise ActiveRecord::RecordNotFound, "Counter-Strike 2 game not found" if game.nil?

      all_results = EventScrapers::CounterStrike2EventScraper.new.call
